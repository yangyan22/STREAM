...
class STREAM(pl.LightningModule):
    ...
    def prompt_wrap(self, img_embeds, atts_img, retriver_text):
        prompt=f'Human: <Img><ImageHere></Img> {self.prompt} '
        batch_size = img_embeds.shape[0]
        p_before, p_after = prompt.split('<ImageHere>')   # Human: <Img>  #  </Img> {self.prompt} \nAssistant:
        ...
        retriver_text_tokens = self.llama_tokenizer(
            retriver_text,
            return_tensors="pt",
            padding="max_length",
            truncation=True,
        ...
        retriver_text_embeds = self.embed_tokens(retriver_text_tokens.input_ids)
        pp = '\nAssistant:'
        pp_tokens = self.llama_tokenizer(
            pp, return_tensors="pt", add_special_tokens=False).to(img_embeds.device)
        pp_after_embeds = self.embed_tokens(pp_tokens.input_ids).expand(batch_size, -1, -1)
        wrapped_embeds = torch.cat([wrapped_img_embeds, retriver_text_embeds, pp_after_embeds], dim=1)
        wrapped_atts = torch.cat([wrapped_atts_img, retriver_text_tokens.attention_mask, pp_tokens.attention_mask.expand(batch_size, -1)], dim=1)  # [attns_p_before, att_and_start_end, [i], attns_p_after]
    ...
    def process_image_sets(self, main_embeds, temp_embeds1, temp_embeds2):
        ...
        new_embed = torch.cat([new_image_embeds, embeddins_temp1 + embeddins_temp2], dim=1) 
        return new_embed
    ...
    def forward(self, samples):
        ...
        retriver_text = samples["retriver_text"]

        bs = len(retriver_text)
        img_embeds, atts_img = self.encode_img(image, bs, temp_images1, temp_images2)
        img_embeds = self.layer_norm(img_embeds)
        img_embeds, atts_img = self.prompt_wrap(img_embeds, atts_img, retriver_text)
