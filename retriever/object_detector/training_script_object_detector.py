...
import sys
sys.path.append("/root/autodl-tmp/rgrg-main/")
...
EPOCHS = 20
LR = 1e-3
EVALUATE_EVERY_K_STEPS = 166504  # how often to evaluate the model on the validation set and log metrics to tensorboard (additionally, model will always be evaluated at end of epoch)
...
def train_model(
    ...
    for epoch in range(epochs):
        ...
        for num_batch, batch in pbar:
            ...
            pbar.set_description(f"Epoch {epoch} - Avg Loss: {train_loss / steps_taken:.4f}, Step: {steps_taken}")
